...
# 1.Introduction    
## 1.1 Data resource    
Our data is about movies’ reviews on Amazon, the raw txt file can be downloaded from kaggle by the link below：   https://www.kaggle.com/dm4006/amazon-movie-reviews 
...
We do ANOVA of score and the length of the review text.The F statistics is 1961.403.It is sigificant. Then we do permutation(parallel) of the score, after the permutation, the F statistics is still large and the p-value is smaller than $e^{-5}$. The regression shows that the score and length of text has negative relationship, and the coefficient is sgnificant. 
...
The score of the movie has positive relationship with the rate of helpfulness, which means that the positive reviews will be esier to recieve "helpful" feedback.  

Over the 12 years, the scores that users gave to the movies dropped , which may suggest that people became more strict nowdays  due to the developement of movie industry. The users are too "busy" to write long reviews nowadays. The summary would be another choice when they have comments on movies.
